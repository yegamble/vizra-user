...
cleanup() {
  ...
  rm -f "$repo/playwright.config.missing-project.ts" \
    ...
    "$repo/e2e/specs/__datauri.spec.ts"
  ...
  # D24 switches the upload gate's pixel refusal off, and D25 its data-URI
  # refusal; restore it whatever happens.
...
# --- D24 THE UPLOAD GATE REFUSES THE PIXELS THE RECORDERS WRITE, however they were turned on (PR #10 round 2)
...
# (scripts/ci/redact-artifacts.sh, the pinned step) refuses an image or video in
# the shapes it knows (AGENTS.md, "What the gate refuses, exactly"), exit 4.
# Each route below is measured in what the RUNNER would upload: the pinned gate
# command, then the upload set copied only if the gate succeeded.
...
# --- D25 AN IMAGE THE PAGE INLINES AS A data: URI (PR #10 close, FINDING 9) ---
# The pixel gate reads names and first bytes, and an image a page inlines -
# `<img src="data:image/png;base64,...">`, the shape a blur placeholder takes - is
# neither. An independent verifier served one from a failing spec: the gate said
# OK, and the whole PNG was recoverable, as base64, from the trace's DOM snapshot
# (`1-trace.trace`) and the HTML response it stores (`resources/<sha1>.html`). No
# obfuscation, no leaving the harness: a page, a failure, an upload.
#
# The spec below is that route. Its image is a tiny SYNTHETIC 1x1 PNG, never a
# real one, and the URI is assembled from parts, so the spec's own source - which
# `error-context.md` quotes as a code frame - does not carry it: what the gate has
# to find is what the TRACE kept. It runs with PLAYWRIGHT_NO_COPY_PROMPT=1, as
# the `e2e` job does, so no page snapshot is written and the only thing left to
# refuse is the inlined image.
log "D25 — an image inlined as a data: URI: the gate refuses it, and nothing uploads"
cat > "$repo/e2e/specs/__datauri.spec.ts" <<'DATAURI_SPEC'
// GENERATED BY scripts/e2e/demonstrate.sh — deleted when the script exits.
import { expect, test } from "../harness/test";

// A synthetic 1x1 PNG (70 bytes, generated, not a screenshot), inlined the way a page would.
const PNG_1X1 = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4nGPQyr/wHwAEmQJpkss54gAAAABJRU5ErkJggg==";
const src = ["data", ":", "image", "/png;base64,", PNG_1X1].join("");
test("an inline image, then a failure", async ({ page }) => {
  await page.route("**/__vizra_inline_image", (route) =>
    route.fulfill({ contentType: "text/html", body: `<!doctype html><title>inline</title><h1>inline</h1><img alt="" src="${src}">` }));
  await page.goto("/__vizra_inline_image");
  await expect(page.getByRole("heading", { level: 1, name: "never present" })).toBeVisible({ timeout: 1000 });
});
DATAURI_SPEC

run_route_ci() {
  # run_route_ci SPEC — as run_route, with the page snapshot suppressed as the `e2e` job suppresses it.
  rm -rf "$repo/test-results" "$repo/playwright-report"
  env PLAYWRIGHT_NO_COPY_PROMPT=1 E2E_COVERAGE_FLOOR=off E2E_BASE_URL="$prod_url" \
    npx playwright test "$1" --project="$project" --workers=1 --retries=0
}
datauri_count() {
  # datauri_count DIR — exit 1 when any file, or any .zip member, carries a data:image/ or data:video/ URI.
  # Names what it found; never prints content.
  local dir=$1 files members=0 zip member work
  files=$({ find "$dir" -type f ! -name '*.zip' -exec grep -laiE 'data:(image|video)\\*/' {} + 2> /dev/null || true; } | wc -l | tr -d ' ')
  while IFS= read -r zip; do
    work=$(mktemp -d)
    unzip -qq -o "$zip" -d "$work"
    while IFS= read -r member; do
      [ -n "$member" ] || continue
      echo "  ${zip#"$dir"/}::${member#"$work"/}"
      members=$((members + 1))
    done < <(grep -rlaiE 'data:(image|video)\\*/' "$work" || true)
    rm -rf "$work"
  done < <(find "$dir" -type f -name '*.zip' 2> /dev/null)
  echo "data-URI images: files=$files archive-members=$members"
  [ "$files" -eq 0 ] && [ "$members" -eq 0 ]
}
# The runner's view again - the pinned gate command, the upload only if it
# succeeded - counting inlined images in what would be uploaded.
gate_then_upload_datauris() {
  gate_then_upload || true
  datauri_count "$gated_upload"
}
gated_upload="$(mktemp -d)"

# (1) The gate on: the spec fails, its trace holds the image, and nothing uploads.
half d25a-inline-image-spec-fails-RED 1 "1 failed" -- run_route_ci e2e/specs/__datauri.spec.ts
half d25a-inline-image-kept-by-the-trace-RED 1 "trace.zip::" -- datauri_count "$repo/test-results"
half d25a-gate-refuses-nothing-uploaded-GREEN 0 "data-URI images: files=0 archive-members=0" -- gate_then_upload_datauris
half d25a-gate-refused-for-the-data-uri-GREEN 0 "encoded as a data: URI" \
  -- grep -F "encoded as a data: URI" "$evidence/d25a-gate-refuses-nothing-uploaded-GREEN.txt"

# (2) The controlled mutation: the data-URI refusal switched off. The image UPLOADS.
cp "$gate_file" "$repo/.redact-artifacts.sh.bak"
digest "D25b redact-artifacts.sh BEFORE" "$gate_file"
perl -0pi -e 's/refuse_data_uris\(\) \{\n/refuse_data_uris() {\n  return 0\n/' "$gate_file"
digest "D25b redact-artifacts.sh MUTATED" "$gate_file"
if cmp -s "$gate_file" "$repo/.redact-artifacts.sh.bak"; then
  echo "FAIL d25b: THE MUTATION DID NOT CHANGE redact-artifacts.sh — a demonstration that does not mutate proves nothing"
  fail=$((fail + 1))
fi
run_route_ci e2e/specs/__datauri.spec.ts > /dev/null 2>&1 || true
half d25b-datauri-refusal-off-image-uploaded-RED 1 "uploaded: yes" -- gate_then_upload_datauris
cp "$repo/.redact-artifacts.sh.bak" "$gate_file"
rm -f "$repo/.redact-artifacts.sh.bak"
digest "D25b redact-artifacts.sh RESTORED" "$gate_file"

# (3) Restored: refused again, nothing uploads.
run_route_ci e2e/specs/__datauri.spec.ts > /dev/null 2>&1 || true
half d25c-restored-gate-refuses-again-GREEN 0 "data-URI images: files=0 archive-members=0" -- gate_then_upload_datauris
rm -f "$repo/e2e/specs/__datauri.spec.ts"
rm -rf "$repo/test-results" "$repo/playwright-report" "$gated_upload"
